...
impl OpenFile {
    ...
    fn parse_cursor(fdinfo: &str) -> Option<usize> {
        ...
        re.captures(fdinfo)?.get(1)?.as_str().parse::<usize>().ok()
    }
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        let path_name = OpenFile::path_to_name(
            fs::read_link(format!("/proc/{}/fd/{}", pid, fd))
                .ok()?
                .to_str()?,
        );
        let fd_content = fs::read_to_string(format!("/proc/{}/fdinfo/{}", pid, fd)).ok()?;
        let cursor = OpenFile::parse_cursor(&fd_content)?;
        let mode = OpenFile::parse_access_mode(&fd_content)?;
        Some(OpenFile::new(path_name, cursor, mode))
    }
